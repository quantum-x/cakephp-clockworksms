<?php
	/**
	* Name:  ClockworkSMS - CakePHP Plugin
	*
	...
	*/
	class ClockworkSMS
	{
		...
		protected $from_string = false;
		protected $clockwork = false;
        protected $options = array();

		public function __construct($config = null)
		{
			//If we have no passed configuration, we'll assume that it's been set in configuration directives
			//If that's not the case, we're going to throw an exception.

			if ($config === null || empty($config)) {
					$config = Configure::read('ClockworkSMS');

					if (!isset($config['apiKey'])) {
						throw new ConfigureException(__d('ClockworkSMS', 'apiKey is not set: required'));
					}
			}
			$this->api_key     			= $config['apiKey'];

			if (isset($config['fromString'])) {
                if (strlen($config['fromString']) > 11) {
                    throw new ConfigureException(__d('ClockworkSMS', 'from is not too long: max 11 characters'));
                } else {
                    $this->from_string		= $config['fromString'];
                    $this->config['from'] = $this->from_string;
                }
			}

			$this->clockwork = new Clockwork($this->api_key, $this->config);
		}

        /**
        * Send SMS
        *
        * @desc Dispatches the SMS.
        *
        * @param <int> Phone Number
        * @param <string> Text Message
        * @param <string> From number / text string. If numeric, cropped to 12. If a string, cropped to 11
        */
		public function send($to = false, $message = false, $from = null)
		{
            $requestArray = array();

			//Do some error checking
			if ($to === false || !is_numeric($to)) {
				throw new BadRequestException(__d('ClockworkSMS', 'Variable: to required, but not set'));
			} else {
                $requestArray['to'] = $to;
            }

			if ($message === false || strlen($message) === 0) {
				throw new BadRequestException(__d('ClockworkSMS', 'Variable: message required, but not set'));
			} else {
                $requestArray['message'] = $message;
            }

			//Check to see if we have 'From' set. If we do, we're going to trim / prepare it as necessary
			if ($from !== false) {
				if (is_numeric($this->from_string))	{
					$this->from_string = substr($this->from_string, 0, 12);
				} else {
					$this->from_string = substr($this->from_string, 0, 11);
				}

				$requestArray['from'] = $from;
			}

			//Check to see if our message is more than 160 chars. If so, we're going to set the 'long' flag
			if (strlen($requestArray['message']) > 160) {
				$requestArray['long'] = 1;
			}

			//Truncate the text message as required
			if (strlen($requestArray['message']) > 463) {
                $requestArray['message'] = substr($requestArray['message'], 0, 463);
			}

			$result = $this->clockwork->send( $requestArray );

			if (isset($result['success'])  && $result['success'] === 1) {
				return true;
			} else {
				throw new InternalErrorException($result['error_message']);
			}
